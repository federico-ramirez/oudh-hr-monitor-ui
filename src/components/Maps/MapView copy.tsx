...
import { MapContainer, Marker, Popup, TileLayer, GeoJSON, Circle, LayerGroup, FeatureGroup } from 'react-leaflet'
...
export default function MapView({ resultados }: MapViewProps) {
    ...
    console.log(resultadosLugaresSet)
    ...
    const filteredMunicipios = distritosCoordinatesData.filter(district => {
        const municipioValue = district.municipio;
        if (!municipioValue) return false;

        const normalizedMunicipioValue = municipioValue
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
        //.toLowerCase();
        return resultadosLugaresSet.has(normalizedMunicipioValue);
    });
                        ...
                        <div id='map-container' ref={mapaRef}>
                            <MapContainer
                                                            ...
                                                            (() => {
                                                                ...
                                                                return (
                                                                    <>
                                                                        ...
                                                                        Tema(s): <span className='font-bold'>{derechosList}</span>
                                                                        <br />
                                                                        Distrito: <span className='font-bold'> {district.distrito} </span>
                                                                        <br />
                                                                        Municipio: <span className='font-bold'> {district.municipio} </span>
                                                                        <br />
                                                                        Departamento: <span className='font-bold'> {district.departamento} </span>
                                                                    </>
                                                                );
                                                            })()
                                                        }
                                ...
                                {filteredMunicipios.length > 0 &&
                                    filteredMunicipios.map((district, index) => {
                                        const position = district.position as [number, number];
                                        ...
                                        return position ? (
                                            <FeatureGroup key={index} pathOptions={{ color: "#f55505" }}>
                                                <Circle center={position} radius={4000} />
                                                <Popup >
                                                    <span className='text-indigo-950 inter-font'>
                                                        {(() => {
                                                            const municipio = district.municipio;
                                                            const normalizedeShapeName = municipio
                                                                ?.normalize("NFD")
                                                                .replace(/[\u0300-\u036f]/g, "")
                                                                .toLowerCase();
